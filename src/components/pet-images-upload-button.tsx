...
export default function PetImagesUploadButton({
  imagePrefix,
  ...props
}: PetImagesUploadButtonProps) {
  ...
  async function handleUploadFiles() {
    ...
    const tempImagesUploadingStatus = images.map(() => {
      return { uploadingStatus: UploadingStatus.Uploading };
    });
    setImagesUploadingStatus(tempImagesUploadingStatus);

    const onUpdateStatus = function (
      status: { uploadingStatus: UploadingStatus }[]
    ) {
    ...
    await Promise.all(
      images.map(async (image, index) => {
        ...
        const { name, res } = await ossClient.put(
          `${imagePrefix}${imageFile.name}`,
          imageFile
        );
        if (res.status === 200) {
          console.log(`Upload Succeed. Name: ${name}`);
          tempImagesUploadingStatus[index].uploadingStatus =
