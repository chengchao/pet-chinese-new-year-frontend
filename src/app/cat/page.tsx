...
export default function Page() {
  ...
  async function uploadFiles(imageList: ImageListType) {
    if (petName === "") {
    ...
    const toDelete = getDifference(filesOnServer, filesShowed)
    console.log(`To delete: ${toDelete}`)
    const toPut = getDifference(filesShowed, filesOnServer)
    ...
    await Promise.all(imageList
      .filter((image) =>
        ...
        newClient = assertValid(newClient)
        const imageFile = assertValid(image.file)
  ...
  async function handleTrain(imageList: ImageListType) {
    await uploadFiles(imageList)
    const imageServerClient = getImageServerClient()
    const names = imageList.map((image) => image.file?.name || image.name).filter((fileName): fileName is string => !!fileName)
    await imageServerClient.sendImagePaths(petName, names)
  }

  function assertValid<T>(client: T | null) {
    if (!client) {
